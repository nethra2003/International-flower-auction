...
if(isset($_POST['name'])) {
    ...
    $sql2 = $con->prepare("INSERT INTO seller (fullname, dob, contactnumber, semail, address, paymentid) VALUES (?, ?, ?, ?, ?, ?)");
    $sql2->bind_param("ssssss", $name, $dob, $contactNumber, $semail, $address, $paymentid);
    $sql2->execute();
    
    //insert into users table
    //get sellerID
    $bid_q = "SELECT sellerId FROM seller ORDER BY sellerId DESC LIMIT 1";
    $res = $con->query($bid_q);
    $bid = $res->fetch_assoc()['sellerId'];
